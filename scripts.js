...
const refreshTokenBalance = async function(tokenAddress, tokenBalanceElement, walletAddress, tokenName, decimals) {
	...
	loadTokenContract(tokenAddress)
		...
		.then(function(value) {
			...
			if (vl > decimals) {
				vl -= decimals;
				value = value.substring(0, vl) + "." + value.substring(vl)
					.padEnd(decimals, "0");
			} else {
...
const conv2dec = function(value, decs) {
	...
	if (vl > decs) {
		vl -= decs;
		return value.substring(0, vl) + "." + value.substring(vl)
			.padEnd(decs, "0");
	} else {
...
const reloadWallet = async function() {
	...
	switch (networkId) {
		case 24734:
			...
			web3.eth.getBalance(walletAddressRAW)
				.then(function(value) {
		...
		case 4:
			...
			tempTokenContract.methods.pendingDividends(walletAddressRAW)
				.call()
				.then(function(value) {
			...
			tempTokenContract.methods.stakedForDividends(walletAddressRAW)
				.call()
				.then(function(value) {
			...
			tempTokenContract.methods.burnedForDividends(walletAddressRAW)
				.call()
				.then(function(value) {
		...
		if (value.length == 2 && decimals >= value[1].length) {
			...
			if (bigpart1.toString() == value[0]) {
				...
				if (bigpart2.toString() == bigpartREV) {
					...
					if (bigpart3.toString()
						.padStart(decimals, "0") == bigpartREV) {
						ret = bigpart1.mul(bigpart2)
							.add(bigpart3)
							.toString();
					}
				}
...
const NativeSend = async function() {
	...
	var realNativeSend = function(value) {
		...
		loadedAccount.signTransaction(transaction)
			.then(function(value) {
...
const sendeubitx = async function(meth) {
	...
	try {
		...
		if (tempconv == "invalid") {
			...
			transaction.data = loadedTokenContracts[contractAddress2].methods.transferFrom(approvalOwner.value, send2, tempconv)
				.encodeABI();
		} else {
			transaction.data = loadedTokenContracts[contractAddress2].methods[meth](send2, tempconv)
				.encodeABI();
		}
	} catch (error) {
		walletMessage.innerHTML = "Invalid address!";
	...
	web3.eth.estimateGas(transaction)
		.then(AfterGasEstimate, function(error) {
...
const ManageDividends = async function(action) {
	...
	switch (action) {
		case "withdraw":
			transaction.data = loadedTokenContracts["0x8e4d858128c9ba2d3a7636892268fab031eddaf8"].methods.withdrawDividendsTo(walletAddressRAW, tempvalue, "0x00")
				.encodeABI();
			break;
		case "unstake":
			transaction.data = loadedTokenContracts["0x8e4d858128c9ba2d3a7636892268fab031eddaf8"].methods.unstake(walletAddressRAW, tempvalue)
				.encodeABI();
			break;
		case "stake":
			transaction.data = loadedTokenContracts["0x8e4d858128c9ba2d3a7636892268fab031eddaf8"].methods.stakeForDividends(walletAddressRAW, tempvalue)
				.encodeABI();
			break;
		case "burn":
			transaction.data = loadedTokenContracts["0x8e4d858128c9ba2d3a7636892268fab031eddaf8"].methods.burnForDividends(walletAddressRAW, tempvalue)
				.encodeABI();
			break;
		default:
			withdrawDividendButton.disabled = false;
...
const checkAllowance = async function() {
	var getSelfAllowance = function(contract, decimals, token) {
		try {
			loadTokenContract(contract)
				.methods.allowance(approvalOwner.value, walletAddressRAW)
				.call()
				.then(function(value) {
					var vl = value.length;
					if (vl > decimals) {
	...
	var getAllowance = function(contract, decimals, token) {
		try {
			loadTokenContract(contract)
				.methods.allowance(walletAddressRAW, sendto.value)
				.call()
				.then(function(value) {
					var vl = value.length;
					if (vl > decimals) {
...
const createRPGF = function() {
	...
	if (value != "invalid") {
		...
		navigator.clipboard.writeText(MintMEReceiverPaidGasFees.methods.sendPreauthorizedTransaction(random, "0x8AFA1b7a8534D519CB04F4075D3189DF8a6738C1", walletAddressRAW, to, value, "115792089237316195423570985008687907853269984665640564039457584007913129639935", temp.v, temp.r, temp.s)
			.encodeABI());
...
const redeemRPGF = async function() {
	...
	//sign and send transaction
	web3.eth.accounts.signTransaction(transaction, privateKeyRAW)
		.then(function(value) {
			web3.eth.sendSignedTransaction(value.rawTransaction)
				.then(function(value) {
					if (value === null) {
...
const PancakeswapApprove = function(address) {
	...
	transaction.data = '0x095ea7b300000000000000000000000010ed43c718714eb63d5aa57b78b54704e256024effffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff';
	//sign and send transaction
	loadedAccount.signTransaction(transaction)
		.then(function(value) {
			web3.eth.sendSignedTransaction(value.rawTransaction)
...
const PancakeSwapTokens = async function() {
	...
	if (PancakeTargetFrom == '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c') {
		...
		transaction.data = PancakeRouter.methods.swapExactETHForTokens(minPancakeOutput, [PancakeTargetFrom, PancakeTargetTo], walletAddressRAW, "115792089237316195423570985008687907853269984665640564039457584007913129639935")
			.encodeABI();
	} else {
		var swapmeth;
		if (PancakeTargetTo == '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c') {
			transaction.data = PancakeRouter.methods.swapExactTokensForETH(PancakeAmountIn, minPancakeOutput, [PancakeTargetFrom, PancakeTargetTo], walletAddressRAW, "115792089237316195423570985008687907853269984665640564039457584007913129639935")
				.encodeABI();
		} else {
			transaction.data = PancakeRouter.methods.swapExactTokensForTokens(PancakeAmountIn, minPancakeOutput, [PancakeTargetFrom, PancakeTargetTo], walletAddressRAW, "115792089237316195423570985008687907853269984665640564039457584007913129639935")
				.encodeABI();
		}
	}
...
const PrePancake = async function() {
	...
	if (PancakeAmountIn != "invalid") {
		PancakeRouter.methods.getAmountsOut(PancakeAmountIn, [PancakeTargetFrom, PancakeTargetTo])
			.call()
			.then(function(value) {
				value = value[1];
				minPancakeOutput = new BigInt(value)
					.div(c100)
					.mul(c99);
